...
- The course's slides are available here: https://github.com/alburezg/kinship_workshop_NUS/tree/master/slides

- Find this website's source code on GitHub: https://github.com/alburezg/kinship_workshop_NUS
...
In the lab sessions we will work with Brazil, so lets see the case of Brazil female population in 2023.
For today session the data is being provided, you will download it later as homework.
...
To choose the country of interest, you can look at the name of the country [here](https://unstats.un.org/unsd/methodology/m49/).
...
Following Caswell [-@caswell_formal_2019], we assume a female closed population in which everyone experiences the Brazilian 2023 mortality and fertility rates at each age throughout their life.
We then ask:
...
We have focused on living kin, but what about relatives who have died during Focal's life? 
...
Use `DemoKin` (assuming time-invariant rates at the 2023 levels in the country of your choice and a female-only population) to explore offspring survival and loss for mothers. 
**Answer**:  What is the expected number of surviving offspring for an average woman aged 65?

```{r 33, include=FALSE}
br_2023 <-
  kin(p = br_surv_2023, f = br_asfr_2023, time_invariant = TRUE)
...
br_2023$kin_summary %>%
  filter(age_focal == 65, kin == "d") %>%
      select(age_focal, loss_2023 = count_cum_dead)
```
...
```{r 35,warning=F, message=F, include=FALSE}
br_2023$kin_summary %>%
...
mean_ages_2023 <-
  br_2023$kin_full %>%
  filter(age_focal == 35, kin == c("m","d","os", "ys")) %>%
...
(Remember we saw on Monday how to download and select the population (N)? We will use it in this section)
...
<!-- Well, because of mortality yes (and also males are older when they have child), but depends of the sex ratio at birth, could change if there are some preference for males and the ratio is not balanced round .5 -->
...
<!-- Yes, in direction. In relative terms there are differences. For example mothers -->
...
kin_out_1950 <- kin2sex(
                      ...
                      time_invariant = TRUE,
                      sex_focal = "f", birth_female = .5)
kin_out_2023 <- kin2sex(
                      pf = br_surv_females[,"2023"],
                      pm = br_surv_males[,"2023"],
                      ff = br_asfr_females[,"2023"],
                      fm = br_asfr_females[,"2023"],
                      time_invariant = TRUE,
                      sex_focal = "f", birth_female = .5)
bind_rows(
...
```{r 64,include=FALSE}
Model_1 <-
  kin(br_surv_females[,"2023"], br_asfr_females[,"2023"], birth_female = .5)$kin_summary %>% 
...
Model_2 <- kin2sex(pf = br_surv_females,
                    ...
                    time_invariant = FALSE,
                    birth_female = .5,
                    output_period = 2023)$kin_summary
```
...
Assignments (one per group) should be sent by email to martins@demogr.mpg.de **before midnight of Friday, November 22.** 
You should hand in the following files:
  ...
  3. All input data needed to replicate your code 
<!-- --------------------------------------------------------------------- -->
